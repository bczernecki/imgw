...
library(raster)
meteo_grid = raster(meteo_mon_sp, ncol = 100, nrow = 100)
mg = gstat(id = "t2", formula = t2~1, data = meteo_mon_sp,
nmax = 7, set = list(idp = .5))
z = interpolate(meteo_grid, mg)
x = meteo_mon_sp %>%
group_split(yy) %>%
purrr::map(~gstat(id = "t2", formula = t2~1, data = ., nmax = 5, set = list(idp = .5))) %>%
purrr::map(~interpolate(meteo_grid, .)) %>%
stack()
aa = as.vector(x[cellFromXY(x, matrix(c(500000, 200000), ncol = 2))])
years = 1966:2018
x_slope = calc(x, fun = function(x) {summary(lm(x ~ years))$coefficients[2]})
tm_shape(europe) +
tm_borders(col = "black", lwd = 2) +
tm_shape(x_slope) +
tm_raster()
...
# library(stars)
# my_grid = st_make_grid(meteo_mon_sp) %>% 
#   st_as_stars()
#   
# v = variogram(exp(t2)~1, meteo_mon_sp)
# m = vgm(45000000, "Sph", 50000, nugget = 1000000)
# plot(v, model = m)
# 
# library(purrr)
# r = meteo_mon_sp %>% 
#   group_split(yy) %>% 
#   map(~krige(formula = exp(t2)~1, ., my_grid, model = m, 
#             nmax = 1))
